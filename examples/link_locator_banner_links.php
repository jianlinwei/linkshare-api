...
$size       = getenv('EXAMPLE_SIZE') ? getenv('EXAMPLE_SIZE') : -1;
...
$apiCaller->makeRequest(
    $linkLocator,
    function (LinkLocator $provider) use ($merchantId, $categoryId, $startDate, $endDate, $size, $campaignId, $page)  {
        $provider->bannerLinks($merchantId, $categoryId, $startDate, $endDate, $size, $campaignId, $page);
    }
);